...
{
    internal class QuickBar : BaseElement
    {
        private float elementSpacing = 6f;
        private List<QuickElement> elements = new List<QuickElement>();

        public override bool Register(BaseElement element)
        {
            if (element is QuickElement)
            {
                ...
                elements.Add((QuickElement)element);
                Calculation();
                return op;
            }
            else
        ...
        public override bool Remove(BaseElement element)
        {
            if (!(element is QuickElement))
                return false;
            var op = elements.Remove((QuickElement)element) && base.Remove(element);
            Calculation();
            return op;
        }
        ...
        public void CalculationQuickElementCenter()
        {
            ...
            {
                QuickElement element = elements[i];
        ...
        public void SwapCenter(int index, int index1)
        {
            if (index == -1 || index1 == -1 || index >= elements.Count || index1 >= elements.Count)
                return;
            QuickElement element = elements[index], element1 = elements[index1];
        ...
        public override void Update(GameTime gt)
        {
            ...
            QuickElement movingElement = null;
            int index = -1;
