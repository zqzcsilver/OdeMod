...
{
    internal class Circle3 : ModProjectile, IMiracleRecorderProj
    {
        ...
        public override void PostDraw(Color lightColor)
        {
            ...
            width = (Projectile.timeLeft + 30)/1.5f;
            for (float i = 1; i <= 60; i++)
            {
                var normalDir = new Vector2((float)Math.Cos(i / 60f * 6.28318f), (float)Math.Sin(i / 60f * 6.28318f));
                bars.Add(new CustomVertexInfo(Projectile.position + normalDir * (width +60+ (60 - Projectile.timeLeft) * 20), color, new Vector3(1, 1, ((float)Projectile.timeLeft / 90f))));
                bars.Add(new CustomVertexInfo(Projectile.position + normalDir * (-width +60+ (60 - Projectile.timeLeft) * 20), color, new Vector3(1, 0, ((float)Projectile.timeLeft / 90f))));
            }
