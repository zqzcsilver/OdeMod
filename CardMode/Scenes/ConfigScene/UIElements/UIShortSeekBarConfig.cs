...
{
    internal class UIShortSeekBarConfig : UIConfigBase<short>
    {
        public readonly short MinValue, MaxValue;
        private UISeekBar seekBar;
        public UIShortSeekBarConfig(string tip, string description, object obj, FieldInfo fieldInfo, short maxValue, short minValue) :
            base(tip, description, obj, fieldInfo)
        {
            MinValue = minValue;
            MaxValue = maxValue;

            short value = (short)fieldInfo.GetValue(obj);
            ...
            {
                fieldInfo.SetValue(obj, (short)(minValue + (maxValue - minValue) * v));
            };
            Register(seekBar);
