...
{
    internal class AbyssShriek : ModProjectile, IHollowKnightProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Projectile.timeLeft == 372)
            {
                ...
                Projectile.position += new Vector2(3f, -180f);
            }
            if (Projectile.timeLeft == 352)
            {
                ...
                for (int i = 1; i <= 50; i++)
                {
                    ...
                    int num2 = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.FireflyHit, 0f, 0f, 0, Color.Black, 2f);
            ...
            if (Projectile.timeLeft >= 0 && Projectile.timeLeft < 352)
            {
                ...
                if (Projectile.timeLeft == 336) Projectile.frame++;
                if (Projectile.timeLeft == 332) Projectile.frame++;
                if (Projectile.timeLeft == 328) Projectile.frame++;
                if (Projectile.timeLeft == 324) Projectile.frame++;
                if (Projectile.timeLeft == 320) Projectile.frame++;
            ...
            if (Projectile.timeLeft == 312)
            {
                ...
                for (int i = 1; i <= 40; i++)
                {
                    int num = Dust.NewDust(Projectile.position, Projectile.width, Projectile.height, DustID.FireflyHit, 0, 0, 0, Color.Black, 2f);
                    Main.dust[num].velocity.Y -= 3f;
                    Main.dust[num].velocity.X *= 0.5f;
                    Main.dust[num].noGravity = true;
                }
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            Texture2D texture = ModContent.Request<Texture2D>("OdeMod/Projectiles/Series/Items/HollowKnight/AbyssShriek").Value;
            ...
            for (int k = 0; k < Projectile.oldPos.Length; k++)
            {
                ...
                Color color = Projectile.GetAlpha(lightColor) * ((float)(Projectile.oldPos.Length - (int)((k + 4) * 1.5)) / Projectile.oldPos.Length);
                Main.spriteBatch.Draw(texture, drawPos, new Rectangle(0, 380 * Projectile.frame, 380, 380), color, Projectile.rotation, drawOrigin, (float)range, SpriteEffects.None, 0f);
            }
