...
{
    internal class UIByteSeekBarConfig : UIConfigBase<byte>
    {
        public readonly byte MinValue, MaxValue;
        private UISeekBar seekBar;
        public UIByteSeekBarConfig(string tip, string description, object obj, FieldInfo fieldInfo, byte maxValue, byte minValue) :
            base(tip, description, obj, fieldInfo)
        {
            MinValue = minValue;
            MaxValue = maxValue;

            byte value = (byte)fieldInfo.GetValue(obj);
            ...
            {
                fieldInfo.SetValue(obj, (byte)(minValue + (maxValue - minValue) * v));
            };
            Register(seekBar);
