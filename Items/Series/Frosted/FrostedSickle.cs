...
namespace OdeMod.Items.Series.Frosted
{
    internal class FrostedSickle : ModItem, IFrosted
    {
        ...
        public override void SetDefaults()
        {
            ...
            Item.autoReuse = true;
            Item.shoot = ModContent.ProjectileType<Projectiles.Series.Items.Frosted.Icelight_>();
            Item.shootSpeed = 12;
        }
        int times = -2;
        Vector2 plrToMouse;
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            if (player.direction == 1)
            {
                ...
                Projectile.NewProjectile(source, player.Center, shootVel, ModContent.ProjectileType<Projectiles.Series.Items.Frosted.Icelight_>(), (int)(damage * 0.5f), knockback, player.whoAmI);
                times++;
                if (times == 1)
                {
                    times = -2;
